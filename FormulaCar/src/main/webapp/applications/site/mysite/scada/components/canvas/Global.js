...
//			}
			...
			for( var i = 0; i < this.images.length; i ++ )
			{
				...
				htmls.push('<img id="canvas-image-'+image[0]+'" src="/FormulaCar/applications/site/mysite/scada/components/canvas/images/resource/'+image[0]+'.png" width="'+image[l]+'" height="'+image[2]+'" />');
			}
				...
				for(var j=0;j<items.length;j++){
					if(items[j].type && items[j].url){
						...
						for(var l=0;l<images.length;l++){
							htmls.push('<img id="canvas-image-'+images[l].el+'" src="/FormulaCar/applications/'+items[j].url+'images/'+images[l].el+'.png" width="'+images[l].width+'" height="'+images[l].height+'" />');
						}
					}
				}
			}
