...
//    },
    {
        "title": "点位名称",
		"lang": "text:automat_list_point_name",
        "dataIndex": "siteName",
        "cls": "date",
        "width": "50%"
    },
			...
			var noticeBar = new NoticeBar({
				...
				events:{
					query:function(opt,name){
                        ...
                        self.name = name;
						self.mask();
                        self.service.getDevicelist(opt,name,0,self.pageDisplay,function(data){
...
//				}
//			});
            this.service.getDevicelist(self.opt,self.name,0,self.pageDisplay,function(data){
				self._renderPagin(data);
			});
		},
		_renderPagin:function(data){
        	...
        	self.paging = new Paging({
				...
				requestData:function(options,success){
                    self.service.getDevicelist(self.opt,self.name,options.cursor,options.limit,function(returnData){
						success(returnData);
					});
				},
