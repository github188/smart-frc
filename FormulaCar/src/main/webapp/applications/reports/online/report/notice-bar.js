...
//			this._setCount();
		...
		_draw:function(){
			var self = this;
			var $html = $(
				"<div class='notice-bar' style='margin-top: 5px;'>"
				...
				+"<div class='notice-bar-state-offline' style='margin-left: 10px;'><p><input value='offline' type='checkbox' id='noticebar-offline-input' class='notice-bar-state-input'/></p><p class='notice-bar-state-text'>"+locale.get({lang:"offline"})//+"</p><p class='notice-bar-state-count'>(<span id='noticebar-offline-count'></span>)</p></div>"
				+"</div>"
...
//				+"<p class='notice-bar-calendar-icon datepicker'></p>"
                ...
                +"<input class='notice-bar-search-input' type='text' style='margin-top: 0px;width: 180px;height:22px' placeholder='"+locale.get('empty_is_all')+"'>"
				+"<div class='notice-bar-calendar-button' id='notice-bar-calendar-button-query' style='margin-left:10px;margin:-3px 0px 0px 6px;line-height: 30px;'></div>"
...
//                }
//            });
            if(permission.app("online_statistics").read){
            	if(this.queryBtn) this.queryBtn.show();
            	if(this.exportButton) this.exportButton.show();
            }else{
            	if(this.queryBtn) this.queryBtn.hide();
            	if(this.exportButton) this.exportButton.hide();
            }
		},
