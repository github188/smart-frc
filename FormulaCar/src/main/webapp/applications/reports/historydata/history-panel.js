define(function(require) {
	...
	var HistoryPanel = Class.create({
		initialize: function(options) {
			...
			this._render();
            this._renderSearchVar();
		},

        _renderSearchVar : function(){
            var self = this;
            var content = $("<div>").addClass("var-search-bar");
            $("<span>").text(locale.get("query") + ":").appendTo(content);
            this.searchInput = $("<input type=\"text\"/>").on("keyup", function(){
                var vars = self._doSearchVar();
                self.varTable.render(vars);

                if(vars && (vars.length > 0)){
                    self._checkCachedVars();
                };
            }).appendTo(content);

            $("#right-table-box-1").prepend(content);
        },

        _checkCachedVars : function(){
            var self = this;
            varCacheList.each(function(one){
                var row = self.varTable.getRowsByProp("varId", one);
                self.varTable.selectRows(row);
            });
        },

        _doSearchVar : function(){
            var value = this.searchInput.val()
            var result = this.varListData;
            if (!this.varListData){
                result = [];
            }else if (value !== null && value !== ""){
                result = this._filtrateVars(this.varListData, value);
            };
            return result;
        },

        _filtrateVars : function(vars, name){
            return vars.findAll(function(one){
                return one.varName.toLowerCase().indexOf(name.toLowerCase()) !== -1;
            });
        },
...
//					}
                    ...
                    this.varListData = varList;
                    varList = this._doSearchVar();
