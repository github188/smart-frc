...
//        	console.log(new Date().getTimezoneOffset());
        ...
        customlogo:function(){
    		...
    		if(indexURL!="www"){
                if(indexURL=="index/whiteboard"){
                ...
                }else if(indexURL=="index/peopledigital"){
                    var urlArr=indexURL.split("/");
                    $("#title").text(locale.get("people_daily"));
                    var href="../"+indexURL+"/favicon.ico";
                    $("#icon-link").attr({
                        "href":href
                    });
                    ...
                    $(".nav-main-corp-content-p2").text(locale.get("people_daily"));
                }else{
                    var urlArr=indexURL.split("/");
                   // $("#title").text(locale.get(urlArr[1]+"+"+"-+device_cloud"));
                    $("#title").text(locale.get("inhand_device_cloud"));
                    var href="../"+indexURL+"/favicon.ico";
                    $("#icon-link").attr({
                        "href":href
                    });
        ...
        _setAccountPosition:function(){
			...
			if(offsetWidth > 1030){
				$("#nav-main-right").css({left:offsetWidth-200});
			}else{
...
//				cloud.storage.sessionStorage("userName",result)
        ...
        _drawNavs : function(one, i, array){
            ...
            //导航项的元素节点
            var el = $("<li style='text-align: center;'>").attr({
                id : one.id,
               // title : locale.get(one.name)
            }).addClass(one.cls)
            .click(function(){
                self.loadNav(one, array, true,true);
            })
            .mouseover(function(){
                self.loadNav(one,array,false,false);
           ...
           // el.append($("<p>")).appendTo(content);
            el.append($("<label style='font-size:16px;line-height: 37px;cursor: pointer;'>"+locale.get(one.name)+"</label>")).appendTo(content);
            one.el = el;
            ...
            if (one.subNavs){           	
            	var subNavCnt = $("<ul>").attr({
                    ...
                    one.subNavs.each(function(subNav, i, subNavs){                  	
                            self._drawSubNavs(subNav, i, subNavs, subNavCnt,one,array);
                    });              
            }
            if(self.roleInfo.oid =="0000000000000000000abcde"){
            	$("#"+one.id).hide();
                if(navStr == "rainbow_system"){
                	$("#"+one.id).show();
        ...
        //绘制具体某一个二级导航项元素节点
        _drawSubNavs :function(subNav, i, subNavs, subNavCnt,nav,navs){
            var self = this;
            ...
            el.click(function(){
                self.loadNav(nav,navs,false,true);
                self.loadSubNav(subNav, navs, true);
            });
            subNav.el = el;
        ...
        unhoverNav : function(navs){
            navs.each(function(one){
                one.el.removeClass("header-icon-on").removeClass(one.cls + "-active").addClass(one.cls);
            });
            this.nav.el.removeClass(this.nav.cls).addClass(this.nav.cls + "-active").addClass("header-icon-on");

            $("#nav-sub-left-app").children().hide();
            this.nav.subNavEl.show();
        },
        //触发选中事件的主导航项的事件处理函数
        loadNav : function(nav, navs, chain,still){
            navs.each(function(one){
               one.el.removeClass("header-icon-on").removeClass(one.cls + "-active").addClass(one.cls);
            });
            ...
            if (chain){
            	...
            	if(subNav){
                    ...
                    self.loadSubNav(nav.subNavs[0],navs,true);
            	}     
            }
        },
        //触发选中事件的二级导航项的事件处理函数
        loadSubNav : function(subNav, navs, chain){
            navs.each(function(nav){
                nav.subNavs.each(function(one){
...
//            });
        ...
        loadApp : function(){
        	...
        	if(nav){
                self.loadNav(nav,self.navs,true,true);
        	}else{
                self.loadNav(self.navs[0],self.navs,true,true);
        	}
        },       
