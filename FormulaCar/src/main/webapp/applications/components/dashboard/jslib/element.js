define(function(require){
	...
	var AgentCanvasTool = Class.create(cloud.Component,{
		...
		initModule:function(draw){
			this.circle = {
				        x: this.drawable.x ? this.drawable.x: this.canvasWidth/2,
	                    y: this.drawable.y ? this.drawable.y : this.canvasHeight/2,
		...
		drawCentroidGuidewire:function(){
			var size=this.drawable.displayScale ? parseInt((this.drawable.displayScale/100) * 70) : 70;
		...
		drawRingOuterCircle:function(){
			...
			this.drawable.board.canvas.ctx2d.shadowOffsetX = 1.5;//设置或返回形状与阴影的水平距离
			this.drawable.board.canvas.ctx2d.shadowOffsetY = 1.5;
