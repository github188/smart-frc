/**
 * Created by zhouyunkui on 14-7-3.
 */
/**
...
//            this.moveToGroupButton.hide();
            this.deleteFromBlackListButton=new Button({
                imgCls:null,
                id:this.moduleName+"-delete-from-black",
                text:locale.get("delete_from_black"),
                lang:"{title:delete_from_black}",
                events:{
                    click:function(obj,args,event){
                        self.fire("deleteBlack",event);
                    }
                }
            });
            this.deleteFromBlackListButton.hide();
...
//                rightItems: [addBtn, deleteBtn]
            ...
            if (this.options.toolbarFeatrues){
                this.toolbar.appendLeftItems([this.selectAllButton,this.createGroupButton,this.moveToGroupButton,this.deleteFromBlackListButton], 0);
        ...
        renderRowBackgroundColor:function(){
            var self=this;
            self.rowsArray=self.content.getRows();
            self.datasArray=self.content.getData();
            self.datasArray.each(function(one,i){
                var temp=self.rowsArray[i];
                if(one.blackOrWhite==1){
                    $(temp).addClass("content-table-black");
                }else{
                    $(temp).removeClass("content-table-black");
                }
            })
        },
        //new page
