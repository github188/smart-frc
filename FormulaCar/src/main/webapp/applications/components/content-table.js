...
//			this.display = Math.ceil((height-60)/30);
        ...
        _renderToolbar: function () {
            ...
            this.createGroupButton = new Button({
                imgCls: null,
                id: this.moduleName + "-create-group",
                text: locale.get("create_group"),
                lang: "{'title':'create_group'}",
                events: {
                    click: function () {
                        self.fire("createGroup");
                    }
                }
            });
            this.createGroupButton.hide();
            this.moveToGroupButton = new Button({
                imgCls: null,
                id: this.moduleName + "-move-to-group",
                text: locale.get("move_to_group"),
                lang: "{'title':'move_to_group'}",
                events: {
                    click: function (obj, args, event) {
                        self.fire("moveToGroup", event);
                    }
                }
            });
            this.moveToGroupButton.hide();
...
//                rightItems: [addBtn, deleteBtn]
            ...
            if (this.options.toolbarFeatrues) {
                this.toolbar.appendLeftItems([this.selectAllButton, this.createGroupButton, this.moveToGroupButton], 0);
            ...
            if (!this.content) {
                this.content = new Table({
                    ...
                    events: {
                        ...
                        onUpdate: function (data, row) {
                            this.fire("afterUpdate", data, row);
                        },
                        scope: this
                    }
...
//            this.mask();
            var defer = this.service.getTableResources(0, this.display, function (data) {
                var total = tag.total;
                this.total = total;
                if(data.result!=null){
...
//            this.updateCountInfo();
        ...
        doDeleteResources: function (resources) {
            ...
            var optional;
            if (this.options.service.resourceType == 14 && this.optionalBtn) {
                optional = this.optionalBtn.isSelected();
            } else if (this.options.service.resourceType == 5 && this.optionalBtn) {
                optional = this.optionalBtn.isSelected();
            }
...
//                alert("At least select one item , Please!");
            ...
            var content;
            if (this.options.service.resourceType == 14 || this.options.service.resourceType == 5) {
                content = [{html: "<span id = \"delete-optional-row\"></span>"}]
            }
//            
            dialog.render({
                lang: "affirm_delete",
            ...
            if (this.optionalBtn) {
                this.optionalBtn.destroy();
            }

            if (this.options.service.resourceType == 14) {
                this.optionalBtn = new Button({
                    container: $("#delete-optional-row"),
                    id: "delete-optional-btn",
                    checkbox: true,
            ...
            } else if (this.options.service.resourceType == 5) {
                this.optionalBtn = new Button({
                    container: $("#delete-optional-row"),
                    id: "delete-optional-btn",
                    checkbox: true,
                    text: locale.get("is_del_site"),
                    lang: "{title:is_del_site, text:is_del_site}",
                    selected: true,
                    events: {
                        click: function () {
                        },
