        	...
        	self.loop = function(){
        		self._apiQueryTask(task["_id"],function(data){
        			...
        			if(state === 3){
        				...
        				if((now - self.task.startTime)/1000 > 120){//2 min
        ...
        _cancelCurrentTask:function(prefix){
        	...
        	if(task){
        		self._apiCancelTask(task["id"], function(data){
        			var result = data.result;
        			self._output(prefix + "Stop the task { " + result["name"] + " - " + result["_id"] + " } successfully");
		        	window.clearInterval(self.t);
		        	self._clearTask();
		        	self._unmask();
        		},function(error){
        			var result = error.result;
        			self._output(prefix + "Stop the task { " + result["name"] + " - " + result["_id"] + " } failure");
        			self._unmask();
        		});
        	}else{
        		self._apiQueryActiveTask(function(data){
        			...
        			if(result.length > 0){
        				...
        				self._apiCancelTask(result[0]["_id"], function(data){
        					var result = data.result;
        					self._output("Stop the task { " + result["name"] + " - " + result["_id"] + " } successfully");
        		        	window.clearInterval(self.t);
        		        	self._clearTask();
        		        	self._unmask();
                		},function(error){
                			var result = error.result;
                			self._output("Stop the task { " + result["name"] + " - " + result["_id"] + " } failure");
                			self._unmask();
                		});
        			}
        		});
...
//        		}
        ...
        _apiCancelTask:function(_id,callback,_callback){
        	Model.task({
        		...
        		success:function(data){
        			if(data.result.state == -1 || data.result.state == 2 || data.result.state == 3){
        				if(callback){
        					callback(data);
        				}
        			}else{
        ...
        _bindEvents:function(){
        	var self = this;
        	$("#normal-mode").children("input").unbind("click").bind("click",function(){
        		self._showNormalMode();
        	});
        	$("#advance-mode").children("input").unbind("click").bind("click",function(){
        		self._showAdvanceMode();
        	});
        	$("#hex").children("input").unbind("click").bind("click",function(){
        		var checked = $(this).attr("checked");
        		if(checked){
        			$("#crlf").children("input").removeAttr("checked");
        		}
        	});
        	$("#crlf").children("input").unbind("click").bind("click",function(){
        		var checked = $(this).attr("checked");
        		if(checked){
        	...
        	$("#remote-control-submit1").unbind("click").bind("click",function(){
        		var value = $("#command-list").val();
        		self.command.send(value);
        	})
        	
        	$("#remote-control-cancel1").unbind("click").bind("click",function(){
        		self._cancelCurrentTask();
        	})
        	
        	$("#remote-control-submit2").unbind("click").bind("click",function(){
        	...
        	$("#remote-control-cancel2").unbind("click").bind("click",function(){
        		self._cancelCurrentTask();
        	})
        	
        	$(".remote-control-cleanscreen").unbind("click").bind("click",function(){
        		$("#remote-control-content").empty();
        	})

            //add by zyl
            $("#remote-control-reboot").unbind("click").bind("click",function(){
                self.command.send("reboot confirmed")
            });
            //add end
        	
        	$(".remote-control-stop-task").die("click").live("click",function(){
        		self._cancelCurrentTask();
        	});
        },
