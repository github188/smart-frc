...
//   var chart = require("../../components/content-chart");
    ...
    var deviceCount = Class.create(cloud.Component,{
...
//                   }
        ...
        setContent : function(){
            var html = "<div class='device-activecount'></div>"
        ...
        setWindowContent:function(){
            this.winCharContainer = $("<div class='device-activecount-chart-container'></div>")
...
//                        lineWidth : 1,
        ...
        getData:function(mask){
            ...
            this.chart = this.element.find(".device-activecount").highcharts()

            this.service.getOnlineDevice(data,function(result){
                self.registData = result;

                var registdeviceChart = self.chart.get("registdevice");
                if(registdeviceChart){
                    registdeviceChart.setData(result)
                }else{
            ...
            this.service.getTotalDevice(data,function(result){
                self.totalData = result;

                var totaldeviceChart = self.chart.get("totaldevice");
                if(totaldeviceChart){
                    totaldeviceChart.setData(result)
                }else{
                    self.chart.addSeries({
                        id:"totaldevice",
                        name: '设备总数',
...
//           console.log(totalData,"totalData");
        ...
        renderChart : function(){
            this.element.find(".device-activecount").highcharts("StockChart",{
                chart: {
                    zoomType: 'x',
                    spacing:[10 ,20, 15, 20]
