define(function(require){
	...
	var columns = [ {
		...
		render: function(data, type, row) {
	       	 ...
	       	 display += new Template(
	                "<div><span id='"+row._id+"' number='"+row.number+"' style='color: #09c;cursor: pointer;' class='invendingunpayb' >"+data+"</span></div>")
	                .evaluate({ 
	                  status: ''
	               });
	...
	var list = Class.create(cloud.Component,{
		...
		_render:function(){
			$("#unpay_list").css("width",$(".wrap").width());
			$("#unpay_list_paging").css("width",$(".wrap").width());
			
			$("#unpay_list").css("height",$("#content-operation-menu").height() - $(".container-hd").height() - $(".main_hd").height());
			
			var listHeight = $("#unpay_list").height();
		    var barHeight = $("#unpay_list_bar").height()*2;
			var tableHeight=listHeight - barHeight - 5;
			$("#unpay_list_table").css("height",tableHeight);
		...
		_renderBtn:function(data){
			var self = this;
        	$(".invendingunpayb").click(function(){
        		var _id = $(this)[0].id;
		...
		_renderTable : function() {
			...
			var height = $("#unpay_list_table").height()+"px";
	        $("#unpay_list_table-table").freezeHeader({ 'height': height });
			this.setDataTable();
		},
		...
		loadTableData : function(limit,cursor) {
			var self = this;
			cloud.util.mask("#unpay_list_table");
			var number = $("#billNo").val();
			var state=[0];
			...
			Service.getOidById(oid,function(datas){
				...
				Service.getUnPayBill(self.searchData,limit,cursor,function(data){
					 console.log(data);
					 var total = data.result.length;
	   	        	 ...
	   	        	 cloud.util.unmask("#unpay_list_table");
				});
			});
			 ...
			 if(self.page){
				 ...
				 self.page = new Paging({
        			...
        			requestData:function(options,callback){
        				cloud.util.mask("#unpay_list_table");
        				Service.getUnPayBill(self.searchData, options.limit,options.cursor,function(data){
