define(function(require){
	...
	var updateWindow = Class.create(cloud.Component,{
		...
		_renderForm:function(){		
	        ...
	        //添加刷新图标
	        $("#ui-window-title").append("<span class='abnormal' ></span><span class='refresh'></span>");
					 ...
					 onRowClick: function(data) {
	                   ...
	                   onRowRendered: function(tr, data, index) {
	                        ...
	                        if(!isNaN(stock) && parseInt(stock) < 0){
	                        	$(tr).find("td").eq(6).html(locale.get({lang:"abnormal_status"}));
	                        	$(tr).find("td").eq(6).css("color","#FFA500");
	                        }
	                    },
	                   scope: this
				}
							 ...
							 var tdata = [];
		        		    	if(data.result.length>0){
		        		    		for(var i=0;i<data.result.length;i++){
		        		    			...
		        		    			var shelves = cidData.replenishDetails;
		        		    			//排序
		        		    			shelves = self.sortByKey(shelves, 'locationId');
		        		    			//在合适位置显示货柜编号
		        						var len = shelves.length;
		        						var index = 0;
		        						if(len%2 == 0){
