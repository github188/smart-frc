define(function(require){
	...
	var AddDeliverPlan = require("./add/addDeliverPlan-window");
		...
		"width" : "15%",
		 render: function(data, type, row) {
             ...
             if (data) {
            	 ...
            	 if(!isNaN(newData) && parseInt(newData) < 0){
            		 title = "出现库存状态是负数的异常状态是补货时未点击补货操作，导致实际售卖数大于上一次补货上报库存数的情况。";
            	 }
            	 display += new Template(
            			 "<div title='"+title+"' style=' background-color:#eee; height:14px;border-radius: 4px; width:120px; border:1px solid #bbb; color:#222;text-align: center;'>"+ 
						  ...
						  var selectedResouces = self.getSelectedResources();
	                 	  ...
	                 	  if (selectedResouces.length === 0) {
	                 		  ...
	                 		  for(var i=0;i<selectedResouces.length;i++){
	                 			  if(selectedResouces[i].stockRate != null){
	                 				 lineIds.push(selectedResouces[i].lineId);
	                 				 lineName.push(selectedResouces[i].lineName);
	                 			  }
	                 			 if(selectedResouces[i].stockRate == null || selectedResouces[i].stockRate == ""){
	                 				 outN ++;
	                 		 ...
	                 		 if(outN == selectedResouces.length){
		 						 dialog.render({lang:"stocks_null"});
		 						 return;
		 					  }else{
		 						 if(this.addDeliverPlan){
			                  			this.addDeliverPlan.destroy();
			                  	  }
			 					  
			 					  this.addDeliverPlan = new AddDeliverPlan({
			                  			selector:"body",
			                  			lineId:lineIds,
			                  			lineName:lineName,
			                  			events : {
