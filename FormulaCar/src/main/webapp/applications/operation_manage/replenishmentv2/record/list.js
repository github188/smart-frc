	                    	...
	                    	var selectedResouces = self.getSelectedResources();
							  if (selectedResouces.length === 0) {
			 					   ...
			 					   return;
		                 		    ...
		                 		    var oid = selectedResouces[0].oid;
			                        ...
			                        var reportName = "ReplenishReport.xlsx";
			                        
			                        var now = Date.parse(new Date())/1000;
			                        var path = "/home/replenish/"+now+"/"+reportName;
			                        var url = host + "/api/vmreports/getTradeExcel?report_name=" + reportName + "&path=" + path + "&access_token=" + cloud.Ajax.getAccessToken();
			                        
			                        Service.createReplenishExcel(oid,now,ids,language,function(data){
			                        		...
			                        		var timer = setInterval(function(){
			                                	...
			                                	Service.findTradeExcel(now,"record.txt",function(data){
			                                		...
			                                		if(data.onlyResultDTO.result.res == "ok"){
			                                    		...
			                                    		$("#"+id).append("<span class='cloud-button-item cloud-button-text'>"+locale.get({lang:"export_replenish_detail"})+"</span>");
			                                    		$("#"+id).show();
			                                		}
			                                	})
