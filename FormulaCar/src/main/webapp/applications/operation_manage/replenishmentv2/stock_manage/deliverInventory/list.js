define(function(require){
	...
	var columns = [ {
		"title":locale.get({lang:"deliver_plan_number"}),//编号
		"dataIndex" : "number",
		"cls" : null,
		"width" : "20%"
	},{                                             
		...
		render:function(data, type, row){
			if(window.location.hostname == "www.dfbs-vm.com"){
				if(data == 0){
					return locale.get({lang:"tobe_ready_goods"});
				}else if(data == 1){
					return locale.get({lang:"tobe_person_unclaimed_goods"});
				}else if(data == 2){
					return locale.get({lang:"deliver"});
				}else if(data == 3){
					return locale.get({lang:"person_back_goods"});
				}else if(data == 4){
					return locale.get({lang:"back_lib"});
				}
			}else{
				if(data == 0){
					return locale.get({lang:"tobe_deliver"});
				}else if(data == 1){
					return locale.get({lang:"deliver"});
				}else if(data == 2){
					return locale.get({lang:"back_lib"});
				...
				events : {
					  ...
					  modify:function(){
	                        ...
	                        if (selectedResouces.length == 0) {
	                        	...
	                        	if(window.location.hostname == "www.dfbs-vm.com"){
	                        		if(type == 4){
	                                	dialog.render({lang: "not_update_unclaimed_goods_bill"});
	    	                            return;
	                                }
	                        	}else{
	                        		if(type == 2){
