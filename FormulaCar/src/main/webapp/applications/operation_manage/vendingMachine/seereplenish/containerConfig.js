define(function(require){
	...
	var config = Class.create(cloud.Component,{
            ...
            for(var i=0;i<saveGoodsConfig.length;i++){
        		...
        		roadInfoHtml = roadInfoHtml + "<td style='width:12.5%;'>"+
        										"<table style='width:100%'>"+
        											...
        											"<tr style='height:40px;width:100%;text-align:center;font-size:10px;'  id='"+saveGoodsConfig[i].location_id+"_goods_name'><td><span style='font-weight:600' id='"+saveGoodsConfig[i].location_id+"_name_content'>&nbsp;"+"</span></td></tr>"+
		...
		initRoadWindowImage:function(data,shelvesState_container){
			var self = this;
			if(data==null||data.length==0){
				...
				$.each(data,function(n,item) {
					if(item!=null){
	        		    ...
	        		    if(shelvesState_container && shelvesState_container.length>0){
                        	for(var j = 0; j < shelvesState_container.length; j++){
                        		if(shelvesState_container[j].shelvesId == item.location_id && shelvesState_container[j].state == 1){
                        			$("#" + item.location_id + "_span").show();
                        		}else if(shelvesState_container[j].shelvesId == item.location_id && shelvesState_container[j].state != 1){
