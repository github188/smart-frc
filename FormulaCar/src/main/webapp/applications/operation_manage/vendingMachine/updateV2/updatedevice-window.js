                        ...
                        if (payStyleData && payStyleData.result.length > 0) {
                            for (var i = 0; i < payStyleData.result.length; i++) {
                                ...
                                if (payStyleData.result[i].name == "wechat") {
                                    ...
                                    name = "融e联";
                                    payType = "icbcpay";
                                }else if (payStyleData.result[i].name == "dragonpay") {
                                    name = "龙支付";
                                    payType = "DRAGONPAY";
                                }
                ...
                success: function(data) {
                	...
                	if(data && data.result){
                    	...
                    	if(self.data && self.data.result.containersNew){
                    		...
                    		for(var x=0;x<self.data.result.containersNew.length;x++){
                    			...
                    			if(modelId != null || modelPicId != null){
                    				if(modelId == 0 || modelId == "0"){
                        				...
                        				$("#cid"+self.cidCount+"_vender").attr("disabled",false);
                        				$("#cid"+self.cidCount+"_machineType").attr("disabled",false);
                    				}
                    		...
                    		if(self.data && self.data.result.containers && self.data.result.createTime == self.data.result.updateTime){
                		...
                		if(data.result.shelves.length>0){
                			...
                			Service.updateAutomat(self.deviceId, subdata, function(data) {
                                if (data.error_code == null) {
                                	var oldSiteName = $("#siteNameOld").val();
	                            	var newSiteName = data.result.siteName;
	                            	if(newSiteName == oldSiteName){
	                            		...
	                            		var events={
	                            				...
	                            				lineId:data.result.lineId,
	                            				lineName:data.result.lineName,
	                            				event_class:17,
	                            				level:1,
	                            				action:1
	                            		};
	                            		Service.addAram(events,function(data){
	                            			console.log(data);
	                            		});
            ...
            if($(".cidinfo").length == 0){
	            ...
	            $(".cidinfo").each(function (m){
	                ...
	                if(modelId == 0 && sef == "disabled"){
	                	if(self.data && self.data.result.containersNew){
                    		for(var x=0;x<self.data.result.containersNew.length;x++){
                    			var fcid = self.data.result.containersNew[x].cid;
                    			if(fcid == cid){
                    				container.shelves = self.data.result.containersNew[x].shelves;
			...
			if(modelId!=null && modelId!=0){
				...
				Service.updateAutomat(self.deviceId, finalData, function(data) {
                    if (data.error_code == null) {
                    	...
                    	var oldSiteName = $("#siteNameOld").val();
                    	var newSiteName = data.result.siteName;
                    	console.log(oldSiteName+"===="+newSiteName);
                    	if(newSiteName == oldSiteName){
                    	}else{//更改点位
                    		var events={
                    				event_type:1,
