        ...
        loadDeviceData: function(map) {
            ...
            Service.getAutomatById(self.deviceId, function(data) {
                ...
                if(masterType==1){
                	...
                	masterType = locale.get({lang: "coffee_machine"});
                }else if(masterType==5){
                	masterType = locale.get({lang: "wine_machine"});
                }
                ...
                if(data.result.inboxConfig){//inbox基本信息
                	if(data.result.adFileList){
                    	self.selfDetailInfo.getData(data.result.inboxConfig,data.result.adFileList);
                    }else{
                    	self.selfDetailInfo.getData(data.result.inboxConfig,null);
                    }
                	
                }else{
                	if(data.result.adFileList){
                    	self.selfDetailInfo.getData(null,data.result.adFileList);
                    }else{
                    	self.selfDetailInfo.getData(null,null);
                    }
                }
