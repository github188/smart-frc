define(function(require){
	...
	var config = Class.create(cloud.Component,{
		...
		sumitButtonClick:function(){
			var self = this;
			$("#goodsconfig_submit2").click(function(){
			    dialog.render({
                    lang: "affirm_modify",
                    buttons: [{
                            lang: "affirm",
            ...
            $("#createTemplate_submit2").bind("click",function(){
		...
		getTab:function(saveGoodsConfig,modelConfigsNew){
			var self=this;
			if(modelConfigsNew != null && modelConfigsNew.modelId != null){
				Service.getModelById(modelConfigsNew.modelId,function(data){
					if(data.result && data.result.shelves){
						var map = {};
						for(var i=0;i<data.result.shelves.length;i++){
							var tempObj = data.result.shelves[i];
							for(var j=0;j<tempObj.length;j++){
								map[tempObj[j].shelvesId] = tempObj[j];
							}
					...
					self.saveGoodsConfig = saveGoodsConfig;
					self.goodsConfig = new goodsConfigInfo({
		...
		getOtherTab:function(containerGoodsConfig){
			...
			if(self.containerGoods && self.containerGoods.length > 0){
				for(var i =0;i<self.containerGoods.length;i++){
					...
					var modelId = self.containerGoods[i].modelId;
					var j=i+1;
					...
					$("#cid_"+cid).bind("click",{cid:cid,shelves:shelves,type:typen,modelId:modelId}, function(e){
						$(".tab_navs li").removeClass("selected");
						...
						if(e.data.modelId != null){
							Service.getModelById(e.data.modelId,function(data){
								if(data.result && data.result.shelves){
									var map = {};
									for(var i=0;i<data.result.shelves.length;i++){
										var tempObj = data.result.shelves[i];
