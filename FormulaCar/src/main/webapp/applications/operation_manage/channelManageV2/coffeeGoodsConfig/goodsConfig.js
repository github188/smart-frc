define(function(require) {
    ...
    var config = Class.create(cloud.Component, {
        initialize: function($super, options) {
            ...
            this.shelvesHtml = {};
            console.log(this.shelvesMap);
            for(var key in this.shelvesMap){
            	var sugar = "";
            	var milk = "";
            	var tempmode = "冷";
            	var measurement = "小";
            	if(this.shelvesMap[key].sugar == "false"){
            		sugar = "style='background:#ddd;'";
            	}
            	if(this.shelvesMap[key].milk == "false"){
        ...
        render:function(){
        	$("#automat_cargo_road_config").css("width",$("#shelf_right").width());
        	$("#automat_cargo_road_config").css("height",$("#component-2").height()-30);
		},
			...
			if(data){
				for (var i = 0; i < data.length; i++) {
	                if(data[i]!=null){
	                    ...
	                    var type = "";
	                    if(self.shelvesMap[data[i].location_id]){
	                    	type = self.shelvesMap[data[i].location_id].goodstype;
	                    }
	                    roadInfoHtml = roadInfoHtml + "<td style='width:17%;'>" +
	                            "<table style='width:100%'>" +
	                            "<tr style='height:82px;width:100%'><td id='" + data[i].location_id + "' class='road_td_image' style='font-size:20px;'></td></tr>" +
	                            "<tr style='width:100%;text-align:center;font-size:10px;'  id='" + data[i].location_id + "_goods_name'><td><div id='"+data[i].location_id+ "_name_div' style='height: 18px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size: 12px;'><span style='font-weight:600' id='" + data[i].location_id + "_name_content'>&nbsp;" + "</span></div></td></tr>" +
	                            "<tr style='width:100%;text-align:center;font-size:10px;' id='" + data[i].location_id + "_goods_price'><td><input type='text' style='width:40px;height: 10px;text-align: center;'  id='" + data[i].location_id + "_price_content' />&nbsp;" + "<span>"+locale.get({lang: "china_yuan"})+"</span><span id='" + data[i].location_id + "_del'></span></td></tr>" +
	                            "<tr style='width:100%;font-size:10px;line-height: 30px;' id='" + data[i].location_id + "_goods_type'><td style='text-align:center'><span  style='width:40px;height: 10px;margin-left: 12px;text-align:center;line-height:16px;'  id='" + data[i].location_id + "_goodstype_content' >类型："+type+"</span></td></tr>"
	                            +"</table>" +
	                            "</td>";
	                    if (i % 6 == 6) {
                ...
                $.each(data, function(n, item) {
                    if (item != null) {
                        ...
                        if (imagepath) {
                            ...
                            $("#" + item.location_id).html("<div style='width: 30px;height: 23px;font-size: 12px;margin-left: 15px;padding-top: 13px;'>" + item.location_id + "</div>+<div class='div_config' style='margin-top:-40px'>"+shelvesHtml+"</div>");
                        }
                        if (item.goods_name) {
                        ...
                        $("#remove_road_"+item.location_id).bind('click',{n:n,location_id:item.location_id,capacity:item.capacity,valve:item.valve},function(e){
                        	...
                        	self.renderGoodsShelf();
		    				$("#"+item.location_id).html("<div style='width: 30px;height: 23px;font-size: 12px;margin-left:15px;padding-top: 13px;position: relative;'>"+item.location_id+"</div>+<div class='div_config' style='margin-top:-40px'>"+shelvesHtml+"</div>");
			...
			Service.getGoodslist("",state,typename,0,24,function(data){
		        ...
		        $(".good_image_hover").click(function(event){
	    			...
	    			for(var ii =0;ii<self.realRoadsData.goodsConfigs.length;ii++){
	    				...
	    				if(self.shelvesMap[roadId]){
	    					var locationId = self.realRoadsData.goodsConfigs[ii].location_id;
		    				var group = self.shelvesMap[roadId].group;
		    				//var group = self.shelvesMap[roadId].group;
		    				var arry = self.group[group];
		    				
		    				if(arry && arry.indexOf(locationId) > -1){
		    					var tempObj = {
		    					...
		    					self.realRoadsData.goodsConfigs[ii] = tempObj;
		    	    			...
		    	    			var src= cloud.config.FILE_SERVER_URL + "/api/file/" +imagepath + "?access_token=" + cloud.Ajax.getAccessToken();
		    	    			$("#"+locationId).html("<div style='width: 30px;height: 10px;font-size: 12px;margin-left: 13px;margin-top: 10px;'>"+locationId+"</div><img src='"+src+"' style='height:60px;'/><div class='div_config' style='margin-top:-60px'>"+shelvesHtml+"</div>");
		    	    			...
		    	    			$("#remove_road_"+locationId).bind('click',{ii:ii,roadId:locationId,capacity:capacity,valve:valve},function(e){
		    	    				...
		    	    				self.renderGoodsShelf();
		    	    				$("#"+e.data.roadId).html("<div style='width: 30px;height: 23px;font-size: 12px;margin-left:15px;padding-top: 13px;position: relative;'>"+locationId+"</div>+<div class='div_config' style='margin-top:-40px'>"+shelvesHtml+"</div>");
        			...
        			for(var ii =0;ii<self.realRoadsData.goodsConfigs.length;ii++){
        				if(self.shelvesMap[roadId]){
    	    				...
    	    				if(arry.indexOf(locationId) > -1){
            	    			...
            	    			$("#remove_road_"+locationId).bind('click',{ii:ii,roadId:locationId},function(e){
            	    				var removeconfig = {
            	                			alipay_url: "",
            	                			goods_id: undefined,
            	                			goods_name: undefined,
