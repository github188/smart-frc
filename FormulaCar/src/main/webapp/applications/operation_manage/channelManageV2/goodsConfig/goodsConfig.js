    ...
    var config = Class.create(cloud.Component, {
        ...
        print:function(data,cid){
          ...
          $("#print_submit").click(function(){
            ...
            var roadInfoHtml = "<div style='height:640px;overflow: auto;'><table id='road_table' style='width:"+width+";margin-top: 10px;'>";
			if(data){
				for (var i = 0; i < data.length; i++) {
	                if(data[i]!=null){
	                    ...
	                    var src ="http://"+host+"/api/file/" + imagepath + "?access_token=" + cloud.Ajax.getAccessToken();
                        ...
                        "<tr style='height:82px;width:100%'>"+
                          "<td id='" + data[i].location_id + "'  style='border: 1px solid #e7e7eb;text-align: center;'>"+
                            "<div style='width: 20px;height: 20px;font-size: 12px;margin-top: -25px;'>" + data[i].location_id + "</div>"+
                            "<span style='font-weight:600;font-size: 14px;' id='" + data[i].location_id + "_name_content'>&nbsp;" +goodsName+ "</span>"+
                          "</td>"+
                        "</tr>";*/
	                    
	                    roadInfoHtml = roadInfoHtml +
	                    "<tr style='width:100%;text-align:center;font-size:10px;'  id='" + data[i].location_id + "_goods_name'><td><div id='"+data[i].location_id+ "_name_div' style='height: 18px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size: 12px;'><span style='font-weight:600' id='" + data[i].location_id + "_name_content'>&nbsp;"+goodsName + "</span></div></td></tr>" +
				...
				for (var i = 0; i < data.length; i++) {
	                if(data[i]!=null){
	                    ...
	                    roadInfoHtml = roadInfoHtml + "<td style='width:10%;'>" +
	                            ...
	                            "<tr style='width:100%;font-size:10px;line-height: 30px;' id='" + data[i].location_id + "_capacity_threshold'><td><input type='text' style='width:40px;height: 10px;margin-left: 12px;text-align:center;line-height:16px;' value='"+capacity+"' id='" + data[i].location_id + "_capacity_content' title='容量' placeholder='"+locale.get({lang:"shelf_rong"})+"'/></td><td><input type='text' style='width:40px;height: 10px;margin-left: -48px;text-align:center;line-height:16px;' value='" + valve + "' id='" + data[i].location_id + "_valve_content' title='阈值' placeholder='"+locale.get({lang:"shelf_threshold"})+"'/></td></tr>"
	                            +"</table>" +
	                            "</td>";
	                    if (i % 8 == 8) {
	                        roadInfoHtml = roadInfoHtml + "</tr>";
                ...
                $.each(data, function(n, item) {
                    if (item != null) {
                        ...
                        $("#remove_road_"+item.location_id).bind('click',{n:n,location_id:item.location_id,capacity:item.capacity,valve:item.valve},function(e){
                        	...
                        	self.renderGoodsShelf();
		    				$("#"+item.location_id).html("<div style='width: 30px;height: 23px;font-size: 12px;margin-top: -20%;    padding-top: 13px;position: relative;'>"+item.location_id+"</div>+");
      ...
      //打开选择商品窗口
		...
		setGoodsContent:function(roadId,type_data){
			...
			Service.getGoodslist(name,state,'',0,24,function(data){
	    		...
	    		html = html+"</table><a id='load' style='margin-left:44%;position:relative;font-size:15px;line-height:30px;'>加载更多商品</a></div></div>";
