define(function(require){
	...
	var updateWindow = Class.create(cloud.Component,{
		...
		loadData:function(){
			...
			Service.getRecordDetailV2ById(self.recordId,function(data){
				
				if(data && data.result && data.result.shelvesRecord){
					
					self.datas = data.result.shelvesRecord;
					var locationIdOrderA = data.result.shelvesRecord.sort(  
		                function(a, b)  
		                {  
		                    return (parseInt(a.locationId) - parseInt(b.locationId));  
		                }  
		            );
					self.listTable.render(locationIdOrderA);
					self._renderBtn();
