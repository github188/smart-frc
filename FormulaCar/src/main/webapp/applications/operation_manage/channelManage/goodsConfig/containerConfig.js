define(function(require){
	...
	var config = Class.create(cloud.Component,{
		...
		showRoads:function(saveGoodsConfig){
    		...
    		var row = 0;
        	var roadInfoHtml = "<table id='road_table' style='margin-top: 10px;width:"+width+"'>";
 			...
 			if(saveGoodsConfig){
 			    for(var i=0;i<saveGoodsConfig.length;i++){
                    ...
                    if(self.machineType == 3){
                    	capacity = 1;
                    	valve = 0;
                    	capinput = "<input type='text' style='width:40px;height: 10px;margin-left: 36px;text-align:center;line-height:16px;' readonly='readonly' value='"+capacity+"' id='" + saveGoodsConfig[i].location_id + "_capacity_content' placeholder='"+locale.get({lang:"shelf_rong"})+"'/>";
                        valveinput = "<input type='text' style='width:40px;height: 10px;text-align:center;line-height:16px;' readonly='readonly' value='" + valve + "' id='" + saveGoodsConfig[i].location_id + "_valve_content' placeholder='"+locale.get({lang:"shelf_threshold"})+"'/>";
                    }else{
                    	capinput = "<input type='text' style='width:40px;height: 10px;margin-left: 36px;text-align:center;line-height:16px;' value='"+capacity+"' id='" + saveGoodsConfig[i].location_id + "_capacity_content' placeholder='"+locale.get({lang:"shelf_rong"})+"'/>";
                        valveinput = "<input type='text' style='width:40px;height: 10px;text-align:center;line-height:16px;' value='" + valve + "' id='" + saveGoodsConfig[i].location_id + "_valve_content' placeholder='"+locale.get({lang:"shelf_threshold"})+"'/>";
                    }
 	        		...
 	        		roadInfoHtml = roadInfoHtml + "<td style='width:12.5%;'>"+
 	        										"<table style='width:100%'>"+
 	        											"<tr style='height:82px;width:100%'>"+
 	        											   "<td id='"+saveGoodsConfig[i].location_id+"' class='road_td_image' style='font-size:20px;'></td>"+
 	        											"</tr>"+
 	        											"<tr style='height:40px;width:100%;text-align:center;font-size:10px;'  id='"+saveGoodsConfig[i].location_id+"_goods_name'>"+
 	        											   "<td><div id='"+saveGoodsConfig[i].location_id+ "_name_div' style='overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'><span style='font-weight:600' id='"+saveGoodsConfig[i].location_id+"_name_content'>&nbsp;"+"</span></div></td>"+
 	        											"</tr>"+
 	        											"<tr style='width:100%;text-align:center;font-size:10px;'  id='" + saveGoodsConfig[i].location_id + "_goods_price'>"+
 	        											   "<td><input type='text' style='width:40px;height: 10px;'  id='" + saveGoodsConfig[i].location_id+ "_price_content' />&nbsp;" + "<span>"+locale.get({lang: "china_yuan"})+"</span><span id='" + saveGoodsConfig[i].location_id + "_del'></span></td>"+
 	        											"</tr>" +
 	        											"<tr style='width:100%;font-size:10px;line-height: 30px;' id='" + saveGoodsConfig[i].location_id + "_capacity_threshold'>"+
 	        											   "<td>"+
 	        											    "<div style='float:left;'>"+capinput+"</div>"+
 	        											    "<div style='float:left;margin-left:5px;'>"+valveinput+"</div>"+
 	        											    "</td>"+
 	        											"</tr>"+
 	        										"</table>"+
 	        									  "</td>";
				...
				$.each(data,function(n,item) {
					if(item!=null){
	        		    ...
	        		    $("#remove_road_"+item.location_id).bind('click',{n:n,location_id:item.location_id,capacity:item.capacity,valve:item.valve},function(e){
			    			...
			    			$("#"+e.data.location_id).html("<div style='width: 30px;height: 23px;font-size: 12px;margin-top: -20%;margin-left: 25px;    padding-top: 13px;position: relative;'>"+item.location_id+"</div>+");
			    			$("#"+e.data.location_id+"_name_content").html("&nbsp");
			    			$("#"+e.data.location_id+"_del").html("&nbsp");
		    				$("#" + e.data.location_id + "_price_content").val("0");
		    				$("#" + e.data.location_id + "_price_content").css("margin-left","-27px");
