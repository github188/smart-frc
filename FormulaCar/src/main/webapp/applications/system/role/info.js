...
//
        ...
        setValue: function(data) {
        	...
        	var names = ["systemManage","networkManage","remoteMaintenance","remoteControl","alarmProcessing","statusSensor"],
...
//        			console.log("item",item.sort(function(a,b){return a*1 - b*1}));
        ...
        getValue: function() {
        	...
        	var names = ["networkManage","remoteMaintenance","remoteControl","alarmProcessing","statusSensor"];
        	for(var i = 0 ; i < $items.length ; i++) {
        		var $inputs = $items.eq(i).find("input");
        		for(var j = 1 ; j < $inputs.length ; j++) {
        			if($inputs.eq(j).attr("checked")) {
