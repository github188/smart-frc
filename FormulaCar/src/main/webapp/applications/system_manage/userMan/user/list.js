define(function(require) {
	...
	var UserPayWin = require("./payStyle/userPay-window.js");
	var columns = [{
        ...
        render: function(data, type, row) {
        	if(data== 0){
        		 ...
        		 var display="";
              	 display += new Template(
                       "<div  style='line-height: 15px;text-align: Center;font-size: 12px;border-radius: 0px;padding: 3px 10px;color: #fff;border: 1px solid #09c;background-color: #09c;width: 50px;cursor: pointer;'><span id='"+row._id+"'  style='cursor: pointer;' class='userpay' >支付配置</span></div>")
                       .evaluate({ 
                         status: ''
                      });
	...
	var list = Class.create(cloud.Component, {
		...
		_renderBtn:function(data){
		    ...
		    $(".userpay").click(function(){
		    	...
		    	if (this.userPay) {
                    this.userPay.destroy();
                }
                this.userPay = new UserPayWin({
                    selector: "body",
                    userId:userId,
                    events: {
		...
		loadData : function() {
			...
			Service.getAllUserInfo(0, pageDisplay,function(data) {
				console.log(data);
	        	...
	        	if(this.page){
	        		...
	        		this.page = new Paging({
	        			...
	        			requestData:function(options,callback){
	        				cloud.util.mask("#user_list_table");
	        				Service.getAllUserInfo(options.cursor, options.limit,function(data){
   							   callback(data);
   							   self._renderBtn(data);
   							cloud.util.unmask("#user_list_table");
	        				});
