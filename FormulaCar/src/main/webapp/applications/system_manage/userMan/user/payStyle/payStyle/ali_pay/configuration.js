        ...
        bindBtnEvents:function(){
        	...
        	$("#sms-config-save").click(function(){
                    ...
                    Service.getSmartUserById(self.userId,function(data_){
                    	...
                    	Service.getAliPayConfig(name,self.configFlag,areaIds,function(data){
                        	if(data && data.result){//修改
                        		var id= data.result._id;
                        		if(self.configFlag == 1 || self.configFlag == "1"){
                					id = "000000000000000000000000";
                				}
                        		Service.updateAliPayConfig(id,name,config,self.configFlag,areaIds,function(data){
                        			dialog.render({lang:"save_success"});
                                    self.setConfig(data.result);
                                    $("tr").css("border-bottom","0px");
                                });
                        	}else{//添加
                        		Service.createAliPayConfig(name,config,self.configFlag,areaIds,function(data){
                        			dialog.render({lang:"save_success"});
                                    self.setConfig(data.result);
                                    $("tr").css("border-bottom","0px");
                                });
