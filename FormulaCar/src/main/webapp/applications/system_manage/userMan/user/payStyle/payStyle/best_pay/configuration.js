    ...
    var configuration = Class.create(cloud.Component, {
                	...
                	var areaIds = data_.result.area;
                	 Service.getBestPayConfig(name,self.configFlag,self.areaIds, function(data) {
                         if (data && data.result) {//修改
                             ...
                             Service.updateBestPayConfig(id, name, config,self.configFlag,areaIds, function(data) {
                             	dialog.render({lang:"save_success"});
                                 self.setConfig(data.result);
                                 $("tr").css("border-bottom","0px");
                             });
                         } else {//添加
                             Service.createBestPayConfig(name, config,self.configFlag,areaIds, function(data) {
                             	dialog.render({lang:"save_success"});
                                 self.setConfig(data.result);
                                 $("tr").css("border-bottom","0px");
                             });
        ...
        getData: function() {
            ...
            Service.getSmartUserById(self.userId,function(data){
                ...
                Service.getBestPayConfig(name,self.configFlag,areaIds, function(data) {
