define(function(require) {
    ...
    var configuration = Class.create(cloud.Component, {
        ...
        jdConfigManage:function(){
        	 var self = this;
        	 var config = self.getConfig();
             var name = "jdpay";
             var areaIds = self.smartData.area;
             Service.JDConfigManage(name,areaIds,config,function(data){
            	 console.log(data);
             });
        ...
        bindBtnEvents: function() {
            ...
            $("#sms-config-save").click(function() {
              ...
              /*  Service.getJDPayConfig(name,self.configFlag,self.areaIds, function(data) {
                    if (data && data.result) {//修改
                        var id = data.result._id;
                        
                        Service.updateJDPayConfig(id, name, config,self.configFlag,self.areaIds, function(data) {
                        ...
                        Service.createJDPayConfig(name, config,self.configFlag,self.areaIds, function(data) {
                        	dialog.render({lang:"save_success"});
                            self.setConfig(data.result);
                            $("tr").css("border-bottom","0px");
        ...
        getData: function() {
            ...
            var name = "jdpay";
            var userId  = cloud.storage.sessionStorage("accountInfo").split(",")[1].split(":")[1];
            //根据userId判断 是否配置支付方式信息
            Service.getJDconfigByUserId(userId,function(data){
