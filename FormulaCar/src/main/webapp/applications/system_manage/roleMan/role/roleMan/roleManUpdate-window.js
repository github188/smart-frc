define(function(require) {
    ...
    var Apps = require("../../../../../permission/apps");
    var HistoryTrend = Class.create(cloud.Component, {
        ...
        _mergeArray:function(m,n){
        	return $.merge(m,n).sort(function(a,b){
				return a-b;
			});
        },
		...
		_renderData:function(){
			...
			Service.getRoleById(self.id,function(data){
				
				var accept = data.result.privileges.accept;
				console.log(accept);
				self.appMap = {};
	            ...
	            require([CONFIG.permission.gates], function(gates){
					...
					for(var i=0;i<navs.length;i++){
		            	var name = navs[i].name;
		            	var subName = navs[i].subNavs[0].name;
		            	var roleNum = self.gates[name][subName][0];
		            	
		            	if($.inArray(roleNum,accept) > -1){
		            		
		            		$('.iwos-shortlist-item').eq(i).click();
		            		if(navs[i].id == "smartvm-monitor"){
		            			$("#ih_apGis").attr("checked",true);
		            			$("#ih_apGis_r").attr("checked",true);
		            			var read = $.merge(Apps["apGis"]["read"],Apps["default"]);
		            			if(self.appMap["ih_apGis"]){
									self.appMap["ih_apGis"] = self._mergeArray(self.appMap["ih_apGis"],read);
									
								}else{
									self.appMap["ih_apGis"] = read;
								}
		            		}else{
		            			var subModule = navs[i].subNavs[0].subModule;

		            			if(subModule.length>0){
		            				for(var j=0;j<subModule.length;j++){
		            					var appName = subModule[j].name;
		            					if(subModule[j].subApp && subModule[j].subApp.length>0){
				            				
		            						for(var k=0;k<subModule[j].subApp.length;k++){
		                        				
		            							
		            							if(subModule[j].subApp[k].defaultShow){
					            					
					            					if(subModule[j].subApp[k].operation && subModule[j].subApp[k].operation.length>0){
					            						var operation = subModule[j].subApp[k].operation;
					            						var subname = subModule[j].subApp[k].name;
					            						for(var m=0;m<operation.length;m++){
					            							
					            							if(operation[m] == 'r'){
						            							var read = $.merge(Apps[subname]["read"],Apps["default"]).uniq();
						            							if(self._compare(accept,read)){
						            								if(self.appMap["ih_"+subname]){
						            									self.appMap["ih_"+subname] = self._mergeArray(self.appMap["ih_"+subname],read);
						            								}else{
						            									self.appMap["ih_"+subname] = read;
						            								}
						            								
						            								$("#ih_"+subname+"_r").attr("checked",true);
						            								$("#ih_"+subname).attr("checked",true);
						            								
						            							}else{
						            								$("#ih_"+subname+"_r").attr("checked",false);
						            								$("#ih_"+subname).attr("checked",false);
						            								self.appMap["ih_"+subname]=[];
						            								
						            							}
						            							
						            						}else if(operation[m] == 'w'){
						            							
						            							var write = $.merge($.merge(Apps[subname]["read"],Apps["default"]).uniq(),Apps[subname]["write"]);
						            							if(self._compare(accept,write)){
						            								if(self.appMap["ih_"+subname]){
						            									self.appMap["ih_"+subname] = self._mergeArray(self.appMap["ih_"+subname],write);
						            								}else{
						            									self.appMap["ih_"+subname] = write;
						            								}
						            								$("#ih_"+subname+"_w").attr("checked",true);
						            							}
				            				...
				            				if(subModule[j].defaultShow){
				            					
				            					if(subModule[j].operation && subModule[j].operation.length>0){
				            						var operation = subModule[j].operation;
				            						for(var m=0;m<operation.length;m++){
				            							
				            							if(operation[m] == 'r'){
					            							var read = $.merge(Apps[appName]["read"],Apps["default"]).uniq();
					            							if(self._compare(accept,read)){
					            								if(self.appMap["ih_"+appName]){
					            									self.appMap["ih_"+appName] = self._mergeArray(self.appMap["ih_"+appName],read);
					            								}else{
					            									self.appMap["ih_"+appName] = read;
					            								}
					            								
					            								$("#ih_"+appName+"_r").attr("checked",true);
					            								$("#ih_"+appName).attr("checked",true);
					            								
					            							}else{
					            								$("#ih_"+appName+"_r").attr("checked",false);
					            								$("#ih_"+appName).attr("checked",false);
					            								self.appMap["ih_"+appName]=[];
					            								
					            							}
					            							
					            						}else if(operation[m] == 'w'){
					            							
					            							var write = $.merge($.merge(Apps[appName]["read"],Apps["default"]).uniq(),Apps[appName]["write"]);
					            							if(self._compare(accept,write)){
					            								if(self.appMap["ih_"+appName]){
					            									self.appMap["ih_"+appName] = self._mergeArray(self.appMap["ih_"+appName],write);
					            								}else{
					            									self.appMap["ih_"+appName] = write;
					            								}
					            								$("#ih_"+appName+"_w").attr("checked",true);
					            							}
        ...
        removeRolePermission:function(id){
            ...
            for(var i=0;i<navs.length;i++){
            	if(navsname == navs[i].name||(navsname=="app"&&navs[i].name=="wechatApp")){
