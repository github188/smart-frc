define(function(require) {
    ...
    var configuration = Class.create(cloud.Component, {
        ...
        render: function() {
            ...
            if(permission.app("QrcodePay_payment").write){
        ...
        getConfig: function() {
            ...
            var supplierType =  $("#supplierType").find("option:selected").val();
        ...
        setConfig: function(data) { 
            ...
            if(data.config && data.config.supplierType){
            	 $("#supplierType option[value='"+data.config.supplierType+"']").attr("selected","selected");
            }
        ...
        bindBtnEvents: function() {
            ...
            $("#sms-config-save").click(function() {
                ...
                Service.getQrcodepayConfig(name,self.configFlag,self.areaIds, function(data) {
                    if (data && data.result) {//修改
                        var id = data.result._id;
                        if(self.configFlag == 1 || self.configFlag == "1"){
        					id = "000000000000000000000000";
        				}
                        Service.updateQrcodepayConfig(id, name, config,self.configFlag,self.areaIds, function(data) {
                        	dialog.render({lang:"save_success"});
                            self.setConfig(data.result);
                        });
                    } else {//添加
                        Service.createQrcodepayConfig(name, config,self.configFlag,self.areaIds, function(data) {
                        	dialog.render({lang:"save_success"});
                            self.setConfig(data.result);
                        });
                    }
