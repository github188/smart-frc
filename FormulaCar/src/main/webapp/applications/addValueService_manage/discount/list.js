define(function(require){
    ...
    var discountMan = require("./discountMan/discountMan-window.js");
	...
	var columns = [ {
		...
		"title":locale.get({lang:"discount_amount"}),//减价金额
		"dataIndex" : "basicConfig",
		"cls" : null,
		"width" : "15%",
		render:function(data){
			if(data){
				return data.coupon/100+locale.get({lang:"yuan"});
			}
		}
	},{                                             
		"title":locale.get({lang:"discount_threshold"}),//使用门槛
		"dataIndex" : "basicConfig",
		"cls" : null,
		"width" : "20%",
		render:function(data){
			if(data){
				return data.threshold/100+locale.get({lang:"yuan_and_above_the_amount_of_orders_available"});
			}
		}
	},{                                             //开始时间
	...
	var list = Class.create(cloud.Component,{
		initialize:function($super,options){
			...
			this.elements = {
				bar : {
					id : "discount_list_bar",
					"class" : null
				},
				table : {
					id : "discount_list_table",
					"class" : null
				},
				paging : {
					id : "discount_list_paging",
					"class" : null
				}
			};
		    this._render();
		...
		_render:function(){
			$("#discount_list").css("width",$(".wrap").width());
			$("#discount_list_paging").css("width",$(".wrap").width());
			$("#discount_list").css("height",$("#content-operation-menu").height() - $(".container-hd").height() - $(".main_hd").height());
			var listHeight = $("#discount_list").height();
		    var barHeight = $("#discount_list_bar").height()*2;
			var tableHeight=listHeight - barHeight - 5;
			$("#discount_list_table").css("height",tableHeight);
			this._renderTable();
		...
		_renderTable : function() {
			...
			var height = $("#discount_list_table").height()+"px";
	        $("#discount_list_table-table").freezeHeader({ 'height': height });
			this.setDataTable();
		},
		...
		loadTableData : function(limit,cursor) {
			cloud.util.mask("#discount_list_table");
        	var self = this;
        	var name = $("#name").val();
        	if(name){
        	...
        	self.searchData.type = 1;//活动类型 1、立减 2、折扣
			Service.getAllDiscount(self.searchData,limit,cursor,function(data){
	        	 ...
	        	 cloud.util.unmask("#discount_list_table");
			 }, self);
		},
