define(function(require) {
    ...
    var winHtml = require("text!./discountMan.html");
    ...
    var updateWindow = Class.create(cloud.Component, {
        ...
        _renderWindow: function() {
            ...
            this.adWindow = new _Window({
                container: "body",
                title: locale.get({lang: "discount_minus"}),
                top: "center",
                left: "center",
                height: 650,
                width: 1000,
            ...
            $("#dataTo1").append("<label>"+locale.get('to')+"</label>");
            this.adWindow.show();
        ...
        getData:function(){
        	...
        	Service.getDiscountById(self.discountId,function(data){
        		...
        		$("#coupon_count").val(data.result.useConfig.coupon_count==null?"":data.result.useConfig.coupon_count);
        ...
        _renderBtn: function() {
        	...
        	$("#nextBase").bind("click", function() {
                ...
                if(coupon==null||coupon.replace(/(^\s*)|(\s*$)/g,"")==""){
           			dialog.render({lang:"please_enter_discount_coupon"});
           			return;
           		}else if(coupon < 1){
           			dialog.render({lang:"discount_coupon_not_less_than_1_yuan"});
           			return;
           		}
                if(threshold==null||threshold.replace(/(^\s*)|(\s*$)/g,"")==""){
           			dialog.render({lang:"please_enter_discount_threshold"});
           			return;
           		}else if(threshold < 0.01){
           			dialog.render({lang:"discount_threshold_not_less_than_1_yuan"});
           			return;
           		}else if(threshold < coupon || threshold == coupon){
           			dialog.render({lang:"discount_threshold_must_not_less_than_coupon"});
           			return;
           		}
                if(start){
                ...
                if(coupon_count==null||coupon_count.replace(/(^\s*)|(\s*$)/g,"")==""){
           			dialog.render({lang:"please_enter_coupon_count"});
           			return;
           		}else if(!strP.test(coupon_count) ||  coupon_count < 5){
           			dialog.render({lang:"coupon_count_not_less_than_5_yuan"});
           			return;
           		}
                var isFollow='';
