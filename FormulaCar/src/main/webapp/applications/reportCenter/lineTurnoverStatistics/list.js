define(function(require){
	...
	var NoticeBar = require("./notice-bar-line");
	...
	var columns = [ {
		...
		"title":locale.get({lang:"site_count"}),//点位个数
		"dataIndex" : "siteSum",
		"cls" : null,
		"width" : "10%"
	},{
	...
	var list = Class.create(cloud.Component,{
		...
		loadTableData : function(limit,cursor) {
	        ...
	        Service.getReportForLineTurnover(self.searchData, limit,cursor,function(data){
			 ...
			 if(self.page){
				 ...
				 self.page = new Paging({
        			...
        			requestData:function(options,callback){
        				cloud.util.mask("#reports_list_table");
        				Service.getReportForLineTurnover(self.searchData, options.limit,options.cursor,function(data){
			...
			this.noticeBar = new NoticeBar({
				...
				events : {
					  ...
					  exReport: function(){
	                        ...
	                        var reportName = "lineTurnOverReport.xlsx";
	                        var oid = cloud.storage.sessionStorage("accountInfo").split(",")[0].split(":")[1];
	                        var now = Date.parse(new Date())/1000;
	                        var path = "/home/lineTurnOver/"+now+"/"+reportName;
	                        var url = host + "/api/vmreports/getTradeExcel?report_name=" + reportName + "&path=" + path + "&access_token=" + cloud.Ajax.getAccessToken();
	                        Service.createReportCenterOfLineTurnOver(now,startTime,endTime,name,language, reportName,oid,function(data){
	                        	if(data){
	                        		...
	                        		var timer = setInterval(function(){
	                                	Service.findReportCenterOfLineTurnOver(now,"lineTurnOver.txt",function(data){
